...
parser.add_argument(
        "--scheme", type=str, default="boolean",
        help="The masking scheme to simulate")
parser.add_argument(
        "--order", type=int, default=1,
        help="The masking order, i.e., #shares - 1")
parser.add_argument(
...
parser.add_argument(
        "-share_weights", action="store_true",
        help="Whether the branches share the same weights."
        )
parser.add_argument(
...
log.info("***********SIMULATION Masking Scheme Encoded***********")
...
log.info("Drawing according to the {} scheme".format(args.scheme))
train_data = simulation.Group(args.n_draws,
                              args.sigma, 
                              args.order,
                              args.n_bits,
                              n_targets=args.n_targets, 
                              group_op=args.scheme)
val_data = simulation.Group(args.n_val, 
                            args.sigma, 
                            args.order,
                            args.n_bits,
                            n_targets=args.n_targets, 
...
net = models.BBMLP(
        ...
        n_hidden = args.n_hidden * (args.order+1), 
...
net = models.KnownPoI(
        ...
        n_shares=args.order+1, 
        n_bits=args.n_bits, 
        share_weights=args.share_weights, 
        normalize_probs=args.normalize_probs
        ).to(device)
optimizer = torch.optim.Adam(net.parameters(), lr=args.learning_rate)
log.info("Instanciates a KnownPoI model")
log.info(net)
...
# Export history
logging.info("Exporting output data concerning KnownPoI model")
for k, val in losses_train_Net.items():
    filename = os.path.join(output_dir, "losses_train_KnownPoI{}.out".format(k))
    np.savetxt(filename, np.array(val))
for k, val in losses_val_Net.items():
    filename = os.path.join(output_dir, "losses_val_KnownPoI{}.out".format(k))
    np.savetxt(filename, np.array(val))
for k, val in branch_losses_val_Net.items():
    filename = os.path.join(output_dir, "branch_losses_val_KnownPoI{}.out".format(k))
    np.savetxt(filename, np.array(val))

log.info("End of simulation for KnownPoI model.")
###############################################################################
knownEnc = models.GroupNet(
        leakage_dims=train_data.get_shapes(), 
        n_hidden = args.n_hidden, 
        scheme=args.scheme, 
        n_bits=args.n_bits, 
...
optimizer = torch.optim.Adam(knownEnc.parameters(), lr=args.learning_rate)
log.info("Instanciates a KnownEnc model")
log.info(knownEnc)

# Training and monitoring
losses_train, losses_val, branch_losses_val = train_val_loop(knownEnc,
                                                             train_loader,
                                                             test_loader,
                                                             criterion,
                                                             args.num_epochs,
...
# Export history
logging.info("Exporting output data concerning KnownEnc model")
for k, val in losses_train.items():
    filename = os.path.join(output_dir, "losses_train_KnownEnc{}.out".format(k))
    np.savetxt(filename, np.array(val))
for k, val in losses_val.items():
    filename = os.path.join(output_dir, "losses_val_KnownEnc{}.out".format(k))
    np.savetxt(filename, np.array(val))
for k, val in branch_losses_val.items():
    filename = os.path.join(output_dir, "branch_losses_val_KnownEnc{}.out".format(k))
    np.savetxt(filename, np.array(val))

log.info("End of simulation for KnownEnc model.")
###############################################################################
